...
final class WSD_Order_Data_Provider {
    ...
    public function is_valid_month(string $month): bool {
        ...
        $d = DateTimeImmutable::createFromFormat('!Y-m', $month, wp_timezone());
        return $d && $d->format('Y-m') === $month;
    }

    public function is_future_month(string $month): bool {
        return $month > wp_date('Y-m');
    }
    ...
    public function get_orders_for_month(string $month): array {
        ...
        do {
            $result = wc_get_orders([
                ...
                'date_created' => $bounds['start']->format('Y-m-d H:i:s') . '...' . $bounds['end']->modify('-1 second')->format('Y-m-d H:i:s'),
                'limit' => 100,
                'page' => $page,
                'paginate' => true,
                'orderby' => 'date',

...
final class WSD_Report_Service {
    ...
    public function render_html(array $payload, string $mode = 'preview'): string {
        ...
        $html .= $this->metric_table([
            ['Product Sales',$c['productSales'] ?? 0], ['Standard Sales',$c['standardSales'] ?? 0], ['VIP Sales',$c['vipSales'] ?? 0], ['Bundle Sales',$c['bundleSales'] ?? 0],
            ['Standard VPC',$c['standardVpc'] ?? 0], ['Standard Commission',$c['standardCommission'] ?? 0], ['VIP Commission',$c['vipCommission'] ?? 0], ['Bundle Commission',$c['bundleCommission'] ?? 0],
        ], $symbol);
        $html .= '</td></tr>';
        ...
        $html .= $this->metric_table([['Marketing',$c['marketing'] ?? 0], ['Other Costs',$c['otherCosts'] ?? 0], ['Net Earnings',$c['netEarnings'] ?? 0]], $symbol);
    ...
    private function money(float $value, string $symbol): string { return esc_html(number_format($value, 2, '.', ',') . ' ' . $symbol); }
    private function month_label(string $month): string { $d = DateTimeImmutable::createFromFormat('!Y-m', $month); return $d ? $d->format('F Y') : $month; }
}
